export default class HoughTransform
{
    ...
    static ShowHoughLines(source, output, segment, lineDatas)
    {
        ...
        let color = ['yellow', 'orange', 'red', 'green', 'blue', 'indigo', 'violet'];

        for(let i = 0; i < lineDatas.length; i++)
        {
            let r = lineDatas[i][0];
            let angle = lineDatas[i][1];
            let angle2 = lineDatas[i][2];
            ...
            let a = x + (lineLength * Math.cos(angle2 * (Math.PI / 180)))
            let b = y + (lineLength * Math.sin(angle2 * (Math.PI / 180)))
