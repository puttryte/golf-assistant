...
function main()
{
    ...
    images[images.length - 1].onload = function()
    {
        ...
        for(let i = 0; i < results.length; i++)
        {
            ...
            console.log("Ball's Coordinates relative to Ball's Bounding Box => X: " + ballCoor[0] + " Y: " + ballCoor[1]);
            ...
            for(let j = 0; j < putterAngles.length; j++)
            {
                ...
                console.log("\tLine Angle: " + putterAngles[j][1]);
            }
        }
    }
...
{ 
    ...
    {
        segments = IsolateSegments(segments, size[0] / 8, size[0] * 2 / 3)
    }
    ...
    if(segments.length == 1)
    {
        ...
        ShowHoughCircle(ctxs[sequence], ctxs[sequence], segments[0], peaks1, Math.round((segments[0].height) / 4));
        ShowHoughLines(ctxs[sequence], ctxs[sequence], segments[0], peaks2);

        out.push(peaks1, peaks2);
    }
    else if(segments.length == 2)
    {
        ...
        let peaks1 = GetHoughCirclePeaks(houghCircleOutput , 0.99, 5);
        let peaks2 = GetHoughLinePeaks(houghLineOutput , 0.75, 5);

        ShowHoughCircle(ctxs[sequence], ctxs[sequence], segments[0], peaks1, Math.round((segments[0].height) / 2));
        ShowHoughLines(ctxs[sequence], ctxs[sequence], segments[1], peaks2);
    ...
    ShowBorderAndSegment(ctxs[sequence], ctxs[sequence], size[0] / 8, size[0] * 1 / 2, segments)
